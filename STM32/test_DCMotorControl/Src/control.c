...
//追加
static float _VelocityResInt,_VelocityRes;
        ...
        if (isEnabled_Control) {
            ...
            switch (ControlMode) {
                ...
                case VelocityControlMode:
                    printf("%.4f,%.4f,%.4f\r\n", VelocityCmd, VelocityRes,_VelocityRes);
                    break;
...
static inline void MajorControlLoop(void)
{
   ...
   /* if (fmodf_time < 1.0f) {
    ...
    //VelocityControl(5.0f, Kp_v_DEFAULT, Ki_v_DEFAULT);
    ...
    //ここ違う気がする．
    //xd(k) = xd(k-1)+ωf(k-1)*dt←ここはxd(k) = xd(k-1)+ωf(k)*dtなのでは？
    //ωf(k) = 1/Tf*{θ(k)-xd(k)}
    ...
    //↓のような気がする
    _VelocityResInt = (_VelocityResInt + dt_major*PositionRes*Gpd) / (1 + dt_major*Gpd);
    _VelocityRes = Gpd * (PositionRes - _VelocityResInt);
